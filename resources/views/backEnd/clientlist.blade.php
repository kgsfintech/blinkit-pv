      ...
      <div class="card-body">
        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
          ...
          @if (Auth::user()->role_id == 11)
            ...
            <li class="nav-item">
              <a class="nav-link" href="{{ url('staff_vs_store/' .$id) }}">Staff vs Store</a>
            </li>
          @endif
          <li class="nav-item">
            <a class="nav-link" href="{{ url('/clientassetregistersearchinput/' . $id) }}">PV Search</a>
          </li>
          <li class="nav-item">
              ...
              <div class="card-body">
                @if (auth()->user()->role_id != 15)
                  <div class="card-head" style="width:930px;height: 10px;">
                    <b style="float:right;margin-top: -17px;">Edit <a href="{{ route('client.edit', $clientList->id) }}"
                  ...
                  <table class="table display table-bordered table-striped table-hover">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          {{ App\Models\Teammember::select('team_member')->where('id', $clientList->leadpartner)->first()->team_member ?? '' }}
                        </td>
                      </tr>
                      <tr>
              ...
              <table class="table display table-bordered table-striped table-hover basic">
                ...
                <tbody>
                  @foreach ($clientassignment as $clientassignmentDatas)
                    <tr>
                      <td> <a
                          href="{{ url('/viewassignment/' . $clientassignmentDatas->assignmentgenerate_id) }}">{{ $clientassignmentDatas->assignmentgenerate_id }}</a>
                      </td>
                      ...
                      <td>
                        @foreach (DB::table('assignmentmappings')->leftjoin('assignmentteammappings', 'assignmentteammappings.assignmentmapping_id', 'assignmentmappings.id')->leftjoin('teammembers', 'teammembers.id', 'assignmentteammappings.teammember_id')->where('assignmentmappings.id', $clientassignmentDatas->id)->get()
      as $sub)
                          @if ($sub->profilepic == null)
                            <a class="avatar avatar-xs" data-toggle="tooltip" title="{{ $sub->team_member }}">
                              <img src="{{ url('backEnd/image/dummy.png') }}" class="avatar-img rounded-circle"
                                alt="...">
                            @else
                              <a class="avatar avatar-xs" data-toggle="tooltip" title="{{ $sub->team_member }}">
              ...
              @if (auth()->user()->role_id != 15)
                <div class="card-head" style="width:930px;height: 10px;">
                  <b style="float:right;margin-top: -17px;">Add <a
                      href="{{ url('/client/contactedit', $clientList->id) }}" class="btn btn-info-soft btn-sm"><i
          ...
          <div class="tab-pane fade" id="pills-client" role="tabpanel" aria-labelledby="pills-client-tab">
            <div class="table-responsive example">
              <br>
              <div class="card-head" style="width:930px;height: 10px;">
                <b style="float:right;margin-top: -17px;">Add <a data-toggle="modal" data-target="#exampleModal1234"
                  ...
                  @foreach ($clientfile as $clientfileData)
                    <tr>
                      <td><a style="font-size: 16px;" target="blank"
                          href="{{ url('folderlist', $clientfileData->id) }}"><i class="far fa-folder">
                            <b>{{ $clientfileData->name }}</b></i></a></td>
