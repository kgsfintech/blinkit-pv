...
<div class="content-header row align-items-center m-0">
    <nav aria-label="breadcrumb" class="col-sm-4 order-sm-last mb-3 mb-sm-0 p-0 ">
        <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
            <li class="breadcrumb-item"><a href="{{url('project/create')}}">Add project</a></li>
            ...
            <div class="card" style="box-shadow:0 4px 8px 0 rgba(0, 0, 0, 0.2);height:650px;">
                ...
                data-toggle="modal" data-target="#exampleModal12"
                    class="btn btn-info-soft btn-sm">Mail <i class="far fa-envelope"></i></a> -->
                        <b style="float:right;margin-top: -17px;"><a href="{{route('project.edit', $project ->id)}}"  class="btn btn-info-soft btn-sm"> 
...
<hr>
            ...
            ->where('projects.id',$project->id)->select('projects.*','clients.client_name')->get();
                  
                         @endphp
                             <td>@foreach($projectData as $projectDatas) {{ $projectDatas->client_name ??''}}
                             @endforeach
                             </td>
                  ...
                  ->where('projects.id',$project->id)->select('projects.*','teammembers.team_member')->get();
                  
                         @endphp
                             <td>@foreach($projectData as $projectDatas) {{ $projectDatas->team_member ??''}}
                             @endforeach
                             </td>
                                    <td><b>Project Manage : </b></td>
                                    <td>{{ $project->projectmanager }}</td>
                                </tr>
                                <tr>
                                <td><b>Project User : </b></td>
                                    <td>{{ $project->projectuser}}</td>
                                ...
                                <tr>
                                     <td><b>Createdby : </b></td>
                                     <td>{{ App\Models\Teammember::select('team_member')->where('id',$project->createdby)->first()->team_member ?? ''}}</td>
                                     <td></td>
                                     <td></td>
                                    </tr>
